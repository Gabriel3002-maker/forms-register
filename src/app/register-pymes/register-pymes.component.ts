...
@Component({
  selector: 'app-register-pymes',
  imports: [StepsModule, ButtonModule, ReactiveFormsModule, CommonModule],
  templateUrl: './register-pymes.component.html',
  styleUrls: ['./register-pymes.component.css']
})
export class RegisterPymesComponent {
  ...
  constructor(
    ...
    this.formStep1 = new FormGroup({
      ...
      capacityProduction: new FormControl('', Validators.required),
      ...
      documentLegalCertification: new FormControl('', Validators.required),
      certificationExistencia: new FormControl('', Validators.required),
      certificationQuality: new FormControl('', Validators.required),
      licenseOperation: new FormControl('', Validators.required),
      consent_privacy_policy: new FormControl('', Validators.required),
      consent_terms_conditions: new FormControl('', Validators.required),
  ...
  goToStep(step: number) {
    if (step === 1 && this.isSection1Valid()) {
      ...
      this.active = step;
    } else if (step === 3 && this.isSection3Valid()) {
      this.active = step;
    }
  }
  
  
  isSectionValid(step: number): boolean {
    switch (step) {
      case 0:
        return this.isSection1Valid();
      case 1:
        return this.isSection2Valid();
      case 2:
        return this.isSection3Valid();
      default:
        return false;
    }
  }
  ...
  isSection1Valid(): boolean {
    return this.formStep1.get('comercialName')!.valid &&
      ...
      this.formStep1.get('sectorActivity')!.valid &&
      this.formStep1.get('nroEmployeeActivity')!.valid;
  }

  isSection2Valid(): boolean {
    return this.formStep1.get('representName')!.valid &&
  ...
  onSubmit() {
    ...
    // Enviar la solicitud
    this.apiService.createPyme({ params }).subscribe({
